...
import { initMultiplayerQuickMessageWheel } from './multiplayer/quick_message_wheel.js'; import './multiplayer/voice_readiness.js'; import './multiplayer/live_voice.js'; import './multiplayer/live_voice_reliability.js'; import './multiplayer/suspend_resume.js';
...
initializeMultiplayerFoundation(player, {
  ...
  worldAdapter: {
    ...
    getNetworkEnemyWaveState,
    restoreNetworkEnemySnapshot,
  ...
  reviveAdapter: {
    ...
    showToast: showStatusToast,
        ...
        requestTeamGameOver: () => requestTeamEliminationEnd(),
...
window.KHADIJA_MULTIPLAYER_BUILD = 'm5-coop-voice-reliability-r1';
window.KHADIJA_MULTIPLAYER_PATCH = 'm5-coop-voice-reliability-r1';
console.info('[Multiplayer Build] m5-coop-voice-reliability-r1 | protocol 6');
console.info('[Multiplayer Patch] m5-coop-voice-reliability-r1');
...
function setActionKeyState(action, pressed) {
  ...
  if (pressed && (isMultiplayerRefreshGameplayBlocked() || isMultiplayerTabLeaseBlocking())) return;
...
function handleOnlineRunEnded(details = {}) {
  if (onlineRunEndHandled) return;
  onlineRunEndHandled = true;
  onlineDeathReportPending = false;
  coOpMenuOpen = false;
...
function returnToMenu(source = 'pause') {
  if (isOnlineMultiplayerRun()) {
    ...
    endMultiplayerRun({
      ...
      player,
      notifyServer: false,
      preserveFinalSummary: false
    });
    handleOnlineRunEnded({ reason: 'left-room' });
...
if (isMultiplayerRefreshGameplayBlocked() || isMultiplayerTabLeaseBlocking()) {
    Object.keys(frameKeys).forEach((key) => {
      frameKeys[key] = false;
    });
    Object.keys(keys).forEach((key) => {
