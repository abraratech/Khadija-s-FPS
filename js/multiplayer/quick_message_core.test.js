...
assert.equal(quickMessageTypeForDigit('6'), TACTICAL_PING_TYPES.REVIVE);
assert.equal(getQuickMessageDefinition('buy/open this').type, TACTICAL_PING_TYPES.INTERACT);
assert.equal(getQuickMessageDefinition('revive me').type, TACTICAL_PING_TYPES.REVIVE);
assert.equal(isQuickMessageAllowed(TACTICAL_PING_TYPES.DEFEND, { online: true, alive: true }).allowed, true);
assert.equal(isQuickMessageAllowed(TACTICAL_PING_TYPES.DEFEND, { online: true, alive: false }).allowed, false);
assert.equal(isQuickMessageAllowed(TACTICAL_PING_TYPES.REVIVE, { online: true, alive: false }).allowed, true);
...
console.log('POST-FINAL.3 quick_message_core tests passed');
